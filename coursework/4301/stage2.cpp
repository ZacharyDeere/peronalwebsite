...
bool isKeyword(string name) {
    ...
    || name == "if" || name == "then" || name == "else" || name == "repeat" || name == "while" ||
    name == "do" || name == "until";
}
...
string externalToInternal(string name) {
    ...
    throw "reference to undefined variable.1";
}
...
string internalToExternal(string name) {
    ...
    throw "reference to undefined variable. 2";
}
...
int getInternalSymbolTableIndex(string name) {
    ...
    throw "reference to undefined variable. 3";
}
...
void ExecStmts() {
    NextToken();
    while (isNON_KEY_ID(token) || token == "read" || token == "write" || token == "if" || token == "while" || token == "repeat" || token == ";" || token == "begin") {
        ExecStmt();
        if (token == ";")
            NextToken();
    }
...
void ElsePt() {
    ...
    if (token == "end" || isNON_KEY_ID(token) || token == ";" || token == "until" || token == "begin" || token == "while" || token == "if" || token == "repeat" || token == "read" || token == "write") {
...
void EmitThenCode(string operand) {
    if (symbolTable.at(getInternalSymbolTableIndex(operand)).dataType != BOOLEAN)
        throw "boolean expression expected";
    string tempLabel = "L" + to_string(numOfLabels++);
    if (aReg != operand)
        writeToObjectFile("", "LDA", operand, "", "", "");
    string remarks = "if false jump to " + tempLabel;
    writeToObjectFile("", "AZJ", tempLabel, "", "", remarks);
...
void EmitElseCode(string operand) {
    string tempLabel = "L" + to_string(numOfLabels++);
    writeToObjectFile("", "UNJ", tempLabel, "", "", "jump to end if");
    writeToObjectFile(operand, "NOP", "", "", "", "else");
    PushOperand(tempLabel);
    aReg = "";
}

void EmitPostIfCode(string operand) {
    writeToObjectFile(operand, "NOP", "", "", "", "end if");
    aReg = "";
}

void EmitWhileCode() {
    string tempLabel = "L" + to_string(numOfLabels++);
    writeToObjectFile(tempLabel, "NOP", "", "", "", "while");
    PushOperand(tempLabel);
    aReg = "";
}
...
void EmitDoCode(string operand) {
    ...
    if (aReg != operand)
        writeToObjectFile("", "LDA", operand, "", "", "");
    writeToObjectFile("", "AZJ", tempLabel, "", "", "do");
    PushOperand(tempLabel);
    if (operand[0] == 'T' && operand != "TRUE")
        FreeTemp();
    aReg = "";
...
void EmitPostWhileCode(string operand1,string operand2) {
    writeToObjectFile("", "UNJ", operand2, "", "", "end while");
    writeToObjectFile(operand1, "NOP", "", "", "", "");
    aReg = "";
}

void EmitRepeatCode() {
    string tempLabel = "L" + to_string(numOfLabels++);
    writeToObjectFile(tempLabel, "NOP", "", "", "", "repeat");
    PushOperand(tempLabel);
    aReg = "";
}

void EmitUntilCode(string operand1, string operand2) {
    if (symbolTable.at(getInternalSymbolTableIndex(operand1)).dataType != BOOLEAN)
        throw "boolean expression expected";
    if (aReg != operand1)
        writeToObjectFile("", "LDA", operand1, "", "", "");
    writeToObjectFile("", "AZJ", operand2, "", "", "until");
    if (operand1[0] == 'T' && operand1 != "TRUE")
        FreeTemp();
    aReg = "";
}